...
*/ 
   ...
   enable_irq(UART_IRQ);
   clear_pending(UART_IRQ1);
   enable_irq(UART_IRQ1);
   clear_pending(UART_IRQ2);
   enable_irq(UART_IRQ2);
   clear_pending(UART_IRQ3);
   enable_irq(UART_IRQ3);
#else
   clear_pending(UART_IRQ);
   enable_irq(UART_IRQ);
#endif
...
//    enable_irq(UART_IRQ);
    ...
    clear_pending(UART_IRQ);
#if defined( __SAMD51__ )
    disable_irq(UART_IRQ1);
    clear_pending(UART_IRQ1);
    disable_irq(UART_IRQ2);
    clear_pending(UART_IRQ2);
    disable_irq(UART_IRQ3);
    clear_pending(UART_IRQ3);
#endif
